...
export function AccountStatusWorkspace({
  ...
  const lifecycle = data.client.lifecycle_status;
  ...
  const saveAccount = () => mutate("update_account", { display_name: displayName.trim() });
  ...
  return <>
    ...
    {manageOpen ? <div className="drawer-backdrop account-manage-backdrop" onClick={() => setManageOpen(false)}><aside className="account-manage-drawer" role="dialog" aria-modal="true" aria-label="Manage account" onClick={(event) => event.stopPropagation()}>
      ...
      <article className="panel account-admin-card"><div className="panel-head"><div><span className="kicker">ACCOUNT CONTROL</span><h2>Identity & automatic status</h2></div><span className={`status-pill ${lifecycle === "live" ? "good" : "neutral"}`}>{lifecycle}</span></div><label><span>Account name</span><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></label><div className="automatic-status-note"><strong>Meta-derived lifecycle</strong><p>Live requires an active campaign and ad inside campaigns containing <b>DetailEngine B2C</b>{status?.meta_status?.campaign_filter ? <> plus <b>{status.meta_status.campaign_filter}</b></> : null}. A launched account without matching live delivery is paused; an account stays onboarding until its first matching launch.</p><small>{status?.meta_status?.matching_campaigns || 0} matching campaigns · {status?.meta_status?.matching_ads || 0} matching ads</small></div><button disabled={busy === "update_account"} type="button" onClick={saveAccount}>{busy === "update_account" ? "Saving…" : "Save account name"}</button><div className="alias-list"><strong>Aliases</strong>{status?.aliases.length ? status.aliases.map((alias) => <span key={alias.id}>{alias.name}</span>) : <small>No aliases recorded.</small>}</div></article>
