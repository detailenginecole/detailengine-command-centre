...
function PerformanceTab({ data, onOpenAds }: { data: CommandCentreData; onOpenAds: () => void }) {
  ...
  return <>
    ...
    <section className="metric-grid account-metrics"><MetricCard label="Total leads" value={count.format(p.total_leads)} note={`${count.format(p.meta_leads)} recorded by Meta`} /><MetricCard label="Qualified" value={count.format(p.qualified_leads)} note={`${pct(p.qualification_rate)} qualification rate`} tone="blue" /><MetricCard label="Warm transfers" value={count.format(p.warm_transfers)} note={`${pct(p.transfer_rate)} of qualified leads`} tone="orange" /></section>
    <section className="dashboard-grid">
