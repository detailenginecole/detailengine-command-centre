...
export function CommandCentre({ initialData, dataUrl, screen }: { initialData: CommandCentreData; dataUrl: string; screen: Screen }) {
  ...
  return <Shell screen={screen} user={data.workspace.current_user} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen}>
    <div className="page-wrap">
      ...
      {screen === "account" && <AccountWorkspace data={data} refreshing={refreshing} onRange={refresh} onReport={setReportType} onSelectLead={setSelectedLead} onFeedback={() => setFeedbackOpen(true)} onConnectors={() => setConnectorsOpen(true)} />}
    </div>
    ...
    {feedbackOpen && <FeedbackContactModal clientSlug={data.client.slug} onClose={() => setFeedbackOpen(false)} onSaved={() => { setFeedbackOpen(false); refresh(); }} />}
...
function PerformanceTab({ data, onTab, onSelectLead, onFeedback }: { data: CommandCentreData; onTab: (tab: AccountTab) => void; onSelectLead: (lead: Lead) => void; onFeedback: () => void }) {
  ...
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel span-4 greg-summary"><div className="panel-head"><div><span className="kicker">GREG · ADVISE ONLY</span><h2>{latestAudit?.headline || "No audit yet"}</h2></div>{latestAudit && <StatusPill status={latestAudit.health_status} />}</div><p>{latestAudit?.summary || "Greg will produce evidence-backed media buying guidance after the first audit."}</p><div className="recommendation-count"><strong>{data.greg.recommendations.filter((row) => !row.acknowledged_at).length}</strong><span>open recommendations</span></div><button className="full-button" onClick={() => onTab("ads")}>Open Ads Manager + Greg →</button></article>
...
function FeedbackContactModal({ clientSlug, onClose, onSaved }: { clientSlug: string; onClose: () => void; onSaved: () => void }) { const [mode, setMode] = useState<"lookup" | "create">("lookup"); const [phone, setPhone] = useState(""); const [firstName, setFirstName] = useState(""); const [lastName, setLastName] = useState(""); const [results, setResults] = useState<Array<{ id: string; name: string; phone: string }>>([]); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); async function request(action: string, extra: Record<string, unknown>) { setBusy(true); setError(""); try { const response = await fetch("/api/feedback-contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, client_slug: clientSlug, ...extra }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Could not update GHL"); return payload; } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not update GHL"); return null; } finally { setBusy(false); } } async function search() { const payload = await request("search", { phone }); if (payload) setResults(payload.contacts || []); } async function assign(contact: { id: string; name: string; phone: string }) { const payload = await request("assign", { contact }); if (payload) onSaved(); } async function create() { const payload = await request("create", { phone, first_name: firstName, last_name: lastName }); if (payload) onSaved(); }
  return <div className="modal-backdrop" onClick={onClose}><section className="modal-card" onClick={(event) => event.stopPropagation()}><header><div><span className="kicker">FEEDBACK CONTACT</span><h2>Who receives the daily tally?</h2></div><button onClick={onClose}>×</button></header><p>We’ll look up this number inside the client’s GHL subaccount.</p><div className="filter-tabs"><button className={mode === "lookup" ? "active" : ""} onClick={() => setMode("lookup")}>Look up</button><button className={mode === "create" ? "active" : ""} onClick={() => setMode("create")}>Create in GHL</button></div>{mode === "create" && <div className="form-grid"><label><span>FIRST NAME</span><input value={firstName} onChange={(event) => setFirstName(event.target.value)} /></label><label><span>LAST NAME</span><input value={lastName} onChange={(event) => setLastName(event.target.value)} /></label></div>}<label className="full-field"><span>PHONE NUMBER</span><input placeholder="+1 555 000 0000" value={phone} onChange={(event) => setPhone(event.target.value)} /></label>{mode === "lookup" ? <button className="primary-button" onClick={search} disabled={busy || !phone}>{busy ? "Looking…" : "Look up in GHL"}</button> : <button className="primary-button" onClick={create} disabled={busy || !phone || !firstName}>{busy ? "Creating…" : "Create + assign"}</button>}{error && <p className="form-error">{error}</p>}{results.length > 0 && <div className="contact-results">{results.map((contact) => <button key={contact.id} onClick={() => assign(contact)}><span>{initials(contact.name)}</span><div><strong>{contact.name}</strong><small>{contact.phone}</small></div><b>Assign →</b></button>)}</div>}</section></div>;
}
