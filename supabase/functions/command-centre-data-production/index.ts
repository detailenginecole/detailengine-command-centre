...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return json({
      ...
      reports: {
        leads: `${supabaseUrl}/functions/v1/command-centre-report?client_id=${encodeURIComponent(client.id)}&from=${range.from}&to=${range.to}&type=leads`,
        ads: `${supabaseUrl}/functions/v1/command-centre-report?client_id=${encodeURIComponent(client.id)}&from=${range.from}&to=${range.to}&type=ads`,
      },
    });
  } catch (error) {
